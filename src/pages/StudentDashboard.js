...
const StudentDashboard = ({ onLogout }) => {
      ...
      if (userData?.studentId || userData?.uid) {
        ...
        try {
          ...
          if (result.success) {
            ...
            const transformedAttendance = result.recentAttendance.map(
              (record) => ({
            ...
            const transformedSubjects = result.subjectPerformance.map(
              (perf) => ({
    ...
    if (userData?.studentId || userData?.uid) {
      ...
      try {
        ...
        if (attendanceResult.success && gradesResult.success) {
          showSuccess(
            "Sample data added successfully! Refreshing dashboard..."
          );
          ...
          showError(
            "Error adding sample data: " +
              (attendanceResult.error || gradesResult.error)
          );
        }
      } catch (error) {
  ...
  const overallPercentage = Math.round(
    subjectMarks.reduce((sum, subject) => sum + subject.percentage, 0) /
      subjectMarks.length
  );
  ...
  const handleQRScanSuccess = async (sessionData) => {
    ...
    try {
      ...
      showError(
        "Invalid QR code data. Please scan a valid attendance QR code."
      );
      setCurrentStep("home");
      return;
    }
  ...
  const verifyLocation = async (sessionData = null) => {
    ...
    try {
      ...
      const sessionLocation = sessionData
        ? sessionData.location
        : attendanceProcess.sessionData?.location;

      if (!sessionLocation) {
        console.error("❌ Session location not available");
        showError(
          "Session location data not found. Please try scanning the QR code again."
        );
        setCurrentStep("home");
        return;
      }
      ...
      if (verificationResult.isValid) {
        ...
        console.log(
          "   Tolerance allowed:",
          `${verificationResult.tolerance}m`
        );
        ...
        showError(
          `Location Verification Failed\n\n` +
            ...
            `Note: Tolerance was ${
              verificationResult.gpsAccuracyConsidered >
              verificationResult.baseToleranceUsed
                ? "adjusted for GPS accuracy"
                : "kept at base value"
            }\n\n` +
      ...
      if (error.message.includes("denied")) {
        ...
        showError(
          "Location information unavailable. Please check your GPS and try again."
        );
      } else if (error.message.includes("timeout")) {
        showError("Location request timed out. Please try again.");
      } else {
        showError(
          "Location verification failed. Please ensure location is enabled and try again."
        );
      }
      setCurrentStep("home");
    }
  ...
  const handleFaceRecognitionSuccess = (recognitionData) => {
    console.log(
      "🎉 handleFaceRecognitionSuccess called with data:",
      recognitionData
    );
        ...
        <div className="bg-white/70 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-xl border border-white/50">
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs sm:text-sm text-gray-500">
                Across {subjectMarks.length} subjects
              </div>
            </div>
          </div>
        </div>
        ...
        {studentClassrooms.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {studentClassrooms.map((classroom) => (
              <div
                key={classroom.id}
                className="bg-white/60 rounded-2xl p-4 border border-white/50 hover:shadow-md transition-shadow"
              >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full mx-4 text-center">
            ...
            <p className="text-gray-600 mb-6">
              Checking if you're in the correct classroom...
            </p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full mx-4 text-center">
            ...
            <p className="text-gray-600 mb-4">
              ...
              {attendanceProcess.sessionData?.subject}
            </p>
            <div className="bg-green-50 rounded-xl p-4">
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">
                    {attendanceProcess.sessionData?.teacher}
                  </span>
                </div>
                <div className="flex justify-between">
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs sm:text-sm text-gray-600 truncate">
                  {studentData.id} • {studentData.class}
                </p>
              </div>
            </div>
